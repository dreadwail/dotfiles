...
class GoCompletionItemProvider {
            ...
            p.on('close', code => {
                try {
                    ...
                    if (!inputText.match(/package\s+(\w+)/)) {
                        return util_1.guessPackageNameFromFile(filename).then(pkgName => {
    ...
    ensureGoCodeConfigured() {
        let importablePkgsPromise = goPackages_1.getImportablePackages(vscode.window.activeTextEditor.document.fileName).then(pkgMap => this.pkgsList = pkgMap);
