...
function _load_typescript() {
  return _typescript = _interopRequireDefault(require('typescript'));
}

var _jestEditorSupport;

function _load_jestEditorSupport() {
  return _jestEditorSupport = require('jest-editor-support');
}

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function parse(file) {
  const sourceFile = (_typescript || _load_typescript()).default.createSourceFile(file, (0, (_fs || _load_fs()).readFileSync)(file).toString(), (_typescript || _load_typescript()).default.ScriptTarget.ES3);

  const itBlocks = [];
  const expects = [];
  function searchNodes(node) {
    if (node.kind === (_typescript || _load_typescript()).default.SyntaxKind.CallExpression) {
      ...
      if (text === 'it' || text === 'test' || text === 'fit') {
        ...
        if (expectText === 'expect') {
          ...
          if (!expects.some(e => e.start.line === position.start.line && e.start.column === position.start.column)) {
