...
function updateDiagnostics(testResults, diagnostics) {
    ...
    function addTestsError(result, uri) {
        ...
        diagnostics.set(uri, asserts.map(assertion => {
            ...
            const diag = new vscode.Diagnostic(new vscode.Range(line, startColumn, line, endColumn), assertion.terseMessage || assertion.shortMessage || assertion.message, vscode.DiagnosticSeverity.Error);
            diag.source = 'Jest';
            return diag;
        }));
    }
