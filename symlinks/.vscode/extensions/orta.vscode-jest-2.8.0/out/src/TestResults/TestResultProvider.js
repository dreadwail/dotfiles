...
class TestResultProvider {
    ...
    getResults(filePath) {
        ...
        for (const test of itBlocks) {
            const assertion = results.filter(result => result.line >= test.start.line && result.line <= test.end.line)[0] ||
                results.filter(result => result.title === test.name && result.status !== TestReconciliationState_1.TestReconciliationState.KnownFail)[0] ||
                {};
