...
class Remapper {
    ...
    sendKey(keys, modeHandler, vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this._logger.debug(`trying to find matching remap. keys=${keys}. mode=${mode_1.Mode[vimState.currentMode]}. keybindings=${this._configKey}.`);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (remapping.commands) {
                for (const command of remapping.commands) {
                    ...
                    statusBar_1.StatusBar.setText(vimState, `${commandString} ${commandArgs}`);
                }
            }
        });
    }
    findMatchingRemap(userDefinedRemappings, inputtedKeys, currentMode) {
        ...
        for (let sliceLength = startingSliceLength; sliceLength >= range[0]; sliceLength--) {
            ...
            if (userDefinedRemappings.has(keySlice)) {
                ...
                if (currentMode !== mode_1.Mode.Insert) {
...
class InsertModeRemapper extends Remapper {
    constructor(recursive) {
        super('insertModeKeyBindings' + (recursive ? '' : 'NonRecursive') + 'Map', [mode_1.Mode.Insert, mode_1.Mode.Replace], recursive);
    }
}
class NormalModeRemapper extends Remapper {
    constructor(recursive) {
        super('normalModeKeyBindings' + (recursive ? '' : 'NonRecursive') + 'Map', [mode_1.Mode.Normal], recursive);
    }
}
class VisualModeRemapper extends Remapper {
    constructor(recursive) {
        super('visualModeKeyBindings' + (recursive ? '' : 'NonRecursive') + 'Map', [mode_1.Mode.Visual, mode_1.Mode.VisualLine, mode_1.Mode.VisualBlock], recursive);
    }
}
class CommandLineModeRemapper extends Remapper {
    constructor(recursive) {
        super('commandLineModeKeyBindings' + (recursive ? '' : 'NonRecursive') + 'Map', [mode_1.Mode.CommandlineInProgress, mode_1.Mode.SearchInProgressMode], recursive);
    }
}
