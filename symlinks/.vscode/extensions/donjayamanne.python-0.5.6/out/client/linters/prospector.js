...
class Linter extends baseLinter.BaseLinter {
    ...
    runLinter(document) {
        ...
        return new Promise((resolve, reject) => {
            utils_1.execPythonFile(prospectorPath, prospectorArgs.concat(['--absolute-paths', '--output-format=json', document.uri.fsPath]), this.workspaceRootPath, false).then(data => {
                let parsedData;
                try {
