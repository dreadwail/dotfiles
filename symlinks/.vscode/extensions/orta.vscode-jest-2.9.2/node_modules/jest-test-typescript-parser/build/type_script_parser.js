...
function parse(file) {
  const sourceFile = (
    _typescript || _load_typescript()
  ).default.createSourceFile(
    file,
  ...
  function searchNodes(node) {
    if (
      node.kind ===
      (_typescript || _load_typescript()).default.SyntaxKind.CallExpression
    ) {
      ...
      if (text === 'it' || text === 'test' || text === 'fit') {
        const position = getNode(
          sourceFile,
          node,
          new (_jestEditorSupport || _load_jestEditorSupport()).ItBlock()
        );
        ...
        if (expectText === 'expect') {
          const position = getNode(
            sourceFile,
            node,
            new (_jestEditorSupport || _load_jestEditorSupport()).Expect()
          );
          if (
            !expects.some(
              e =>
  ...
  (_typescript || _load_typescript()).default.forEachChild(
    sourceFile,
    searchNodes
  );
  return {
