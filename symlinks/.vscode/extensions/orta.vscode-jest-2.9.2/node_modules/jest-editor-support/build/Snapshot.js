...
const buildName = (snapshotNode, parents, position) => {
  ...
  return (_jestSnapshot || _load_jestSnapshot()).utils.testNameToKey(
    fullName,
    position
  );
};
...
class Snapshot {
  constructor(parser, customMatchers) {
    this._parser =
      parser || (_babylon_parser || _load_babylon_parser()).getASTfor;
  ...
  getMetadata(filePath) {
    ...
    const Visitors = {
      Identifier: function(path, state, matchers) {
    ...
    ).utils.getSnapshotPath(filePath);
    const snapshots = (
      _jestSnapshot || _load_jestSnapshot()
    ).utils.getSnapshotData(snapshotPath, 'none').data;
    let lastParent = null;
    let count = 1;
