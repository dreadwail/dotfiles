...
class ElixirServer {
    ...
    start() {
        ...
        if (vscode.workspace.rootPath !== undefined) {
            ...
            const savedFiles = vscode.workspace.textDocuments.filter((value) => {
        ...
        this.p.stdout.on('data', (chunk) => {
            if (chunk.indexOf(`END-OF-${this.lastRequestType}`) > -1) {
                ...
                const splitStrings = chunkString.split(`END-OF-${this.lastRequestType}`);
    ...
    getDefinition(document, position, callback) {
    ...
    getDocumentation(document, position, callback) {
        const wordAtPosition = document.getWordRangeAtPosition(position);
        const word = document.getText(wordAtPosition);
        if (word.indexOf('\n') >= 0) {
            console.error('[vscode-elixir] got whole file as word');
    ...
    getCompletions(document, position, callback) {
        const wordAtPosition = document.getWordRangeAtPosition(position);
        const word = document.getText(wordAtPosition);
        if (word.indexOf('\n') >= 0) {
            console.error('[vscode-elixir] got whole file as word');
    ...
    createCompletion(hint, line, isErlangModule = false) {
