...
class ConfigManager {
    ...
    static isSingleInstallation() {
        const regex = new RegExp(`(.+[\\|/]extensions[\\|/])(?:.*${constants_1.constants.extension.name})`);
        const matches = path_1.dirname(__filename).match(regex);
        const vscodeExtensionDirPath = (matches && matches.length > 0 && matches[1]) || './';
        const extensionNameRegExp = new RegExp(`.*${constants_1.constants.extension.name}`);
        const existingInstallations = fs_1.readdirSync(vscodeExtensionDirPath).filter(filename => extensionNameRegExp.test(filename)).length;
